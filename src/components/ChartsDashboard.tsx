...
export default function ChartsDashboard({ tasks }: Props) {
  ...
  const roiBuckets = [
    { label: '<200', count: tasks.filter(t => t.roi !== null && typeof t.roi === 'number' && t.roi < 200).length },
    { label: '200-500', count: tasks.filter(t => t.roi !== null && typeof t.roi === 'number' && t.roi >= 200 && t.roi <= 500).length },
    { label: '>500', count: tasks.filter(t => t.roi !== null && typeof t.roi === 'number' && t.roi > 500).length },
